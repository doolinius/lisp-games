...
;;;;        special skills for melee fighters
;;;;        something fun/unique for weapons upgrade, maybe tied with special
;;;;            attacks
;;;;
;;;;        weapon types - sword, two-hand-sword, axe, hammer, bow, crossbow,
;;;;                       spear, lance,
...
;;;;
;;;;        rewrite stat accessors and calculations
;;;;
...
(defparameter *spells*
  (list :fire (make-atk-spell "Fire" 3 2 1 :fire2 7 :fire)
	...
	:fire3 (make-atk-spell "Fire 3" 10 2 2 nil 18 :fire)
...
(defun spell-series (spell-symbol)
  (if (null spell-symbol)
      nil
      (let ((spell (getf *spells* spell-symbol)))
	(cons spell (spell-series (next-level spell))))))

(defparameter *default-teffects*
  (list
   :grass 1.0
   :path 1.0
   :stone 1.0
...
(defclass char-class ()
  ((name :initarg :name :accessor name)
   ...
   (teffects :initarg :teffects :accessor teffects) 
   (class-res :initarg :class-res :initform nil :accessor class-res)
   ))

;;; helper function for creating instances of char-class
(defun make-char-class (name weapons stat-bonuses promotion-classes &key (heal-bonus 1) (teffects '()))
  (make-instance 'char-class :name name :weapons weapons :stat-bonuses stat-bonuses :promotion-classes promotion-classes :heal-bonus heal-bonus :teffects teffects))
...
(defparameter *char-classes*
  (list
   ...
   :ranger    (make-char-class "Ranger" '(bow knife) '(move 1) nil :heal-bonus 2
			       :teffects '(:forest 1.0 :sand 1.0 :brush 1.0 :snow 1.0))
   ...
   :mystic    (make-char-class "Mystic" '(sword) nil nil :heal-bonus 1.5
			       :teffects '(:forest 1.0 :sand 1.0 :brush 1.25))
   :paladin   (make-char-class "Paladin" '(spear lance sword) nil nil :heal-bonus 1.25)
   :cavalier  (make-char-class "Cavalier" '(spear lance) '(move 2) nil)
...
(defclass gcharacter ()
  ((name :initarg :name :accessor name)
   ...
   (teffects :initarg :teffects :accessor teffects :initform '())
...
(defun terrain-effect (gchar te)
  (or (getf (teffects gchar) te)
      (getf (teffects (char-class gchar)) te)
      (getf *default-teffects* te)))

;;; concatenates a symbol and a postfix and returns a function referred to
;;;  by the new string
;;;  I now realize that even though it's cool I can do this, it's a pretty
;;;  ghetto way of solving the problem
...
;;; LEVEL-UP CODE
;;; stats that increase with level-up: STR, DEF, AGI, INT, HP, MP, LUK

;; should look like: (level-up *lerris*)
;; (stat-progressions *lerris*) another class slot
;; (list
...
;;; linear stat progression function
;;; stats increase at a steady rate
;;; factor determines how quickly
(defun linear (factor level)
  ())

;;; slow start progression
;;; stats grow slowly at first then quickly later
;;; characters are great late in the game
(defun slow-start (factor level)
  ())

;;; fast start progression
;;; stats grow quickly at first, but taper off in later levels
;;; characters are good early in the game
(defun fast-start (factor level)
  ())
...
(defun make-character (name char-class level max-hp max-mp str def int agi luk mov crit weapon &key extra resistance teffects)
  (make-instance 'gcharacter :name name :level level :char-class (getf *char-classes* char-class) :max-hp max-hp :hp max-hp :max-mp max-mp :mp max-mp :str str :def def :agi agi :int int :luk luk :mov mov :crit crit :weapon (getf *weapons* weapon) :extra extra :resistance resistance :teffects teffects))

;;; test characters
(defparameter *lerris*
  (make-character "Lerris" :fighter 1 12 10 12 8 10 8 10 5 12 :short-sword
		  :teffects '(:forest 1.5 :snow 1.25)))
...
(defun heal-amount (spell attacker)
  (+ (hp-restore spell) 
     (if (<= (random 100) (intelligence-stat attacker)) 
	 (1+ (random 2)) 
	 0)))
...
;;; returns all four mathematically adjacent coordinates that
;;;   are adjacent to the supplied coordinate
...
(defun walkable (coord map gchar)
  (let ((te (aref map (cdr coord) (car coord))))
    (terrain-effect gchar te)))
...
;;;          map - the terrain map
;;;          gchar - the character whose terrain effects will determine
;;;                  whether the terrain is walkable
(defun adjacent (coord map gchar)
  (remove-if (lambda (c)
	       (or (not-on-map c (array-dimensions map))
		   (not (walkable c map gchar))))
...
(loop for i from 3 to 9
   do (setf (aref *map* i 2) :water))

(defun terrain (map coord)
  (aref map (cdr coord) (car coord)))

(defun move-area (coord gchar map)
  (let ((mov (move gchar)))
    (labels ((next-steps (nc mov)
	       (let ((mov-dec (terrain-effect gchar (terrain map nc))))
		 (if (< mov mov-dec)
		     nil
		     (append (list nc)
			     (mapcan (lambda (c)
				       (next-steps c (- mov mov-dec)))
				     (adjacent nc map gchar)))))))
      (remove-duplicates (append (list coord)
				 (mapcan (lambda (c)
					   (next-steps c mov))
					 (adjacent coord map gchar))) :test #'equal))))
...
(defun a* (gchar map dst)
  (let* ((src (coord gchar))
	(goal (make-node :coord dst))
	...
	(progn
	  ...
	  (if (equal (node-coord q) (node-coord goal))
	      (setf found t)
	      (dolist (successor (mapcar (lambda (c)
					   (make-node :coord c :parent (node-coord q)))
					 (adjacent (node-coord q) map gchar)))
		(progn
		  (setf (node-g successor) (+ (node-g q) (terrain-effect gchar (terrain map (node-coord successor)))));;(1+ (node-g q)))
...
(defun parse-path (nodes)
  (if (not (node-parent (car nodes)))
      nil
      (cons (node-coord (car nodes))
	    (parse-path (nthcdr (position (node-parent (car nodes))
					  nodes :key #'node-coord :test #'equal)
				nodes)))))

(defun find-path (character coord map)
  (reverse (parse-path (a* character map coord))))
